...
본 서비스는 Web Server(nginx), API Server(Django), DB Server(PostgreSQL) 로 구성된 3계층 구조를 차용하여 설계되었으며, 특히 외부망에서는 HTML 기반 UI만 제공하고 내부망에서 실제 로직과 데이터 처리가 이루어지는 DMZ(비무장지대) 아키텍처를 구현하고 있습니다.
